...
public class DynamicChainWithConv {
    ...
    public DynamicChainWithConv(ArrayList<Vertex> Vertices, ArrayList<graphEdge> Edges, ArrayList<functionnode> VNFs,
    ...
    public getFunction getNextNodeFirstGroup(findShortestPathWithConv SP, TreeMap<Integer, ArrayList<Integer>> treemap,
        ...
        for (int i = 0; i < elementsOfGroup.size(); i++) {
            ...
            if (VerticesCurrentVNF.isEmpty()) {
                flagTop = false;
                //////System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                //////System.out.println("Request is rejected because there is no substrate candidate for node: nextNode +  " + currentVNF);
                //////System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            }
...
//                + returnedFromSPNewFinal);
    ...
    public getFunction getNextNodeOtherGroups(findShortestPathWithConv SP, TreeMap<Integer, ArrayList<Integer>> treemap,
...
//                    + " the list: " + currentPair.getReturnSPHop());
            ...
            for (int j = 0; j < currentPair.getReturnSPHop().size(); j++) {
                ...
                //////System.out.println("Node on the path: " + currentPair.getReturnSPHop().get(j) +
...
//                + nextNode.getNumber() + " the list: " + returnedFromSPNewFinal);
    ...
    public int[] mapping() {
        ...
        ArrayList<functionnode> ParentList = new ArrayList<functionnode>();
        ArrayList<functionnode> ChildList = new ArrayList<functionnode>();
        findShortestPathWithConv SP = new findShortestPathWithConv(Vertices, Edges, VNFs,this.substrateNet);
        ...
        ArrayList<ArrayList<Vertex>> mappedChain = new ArrayList<ArrayList<Vertex>>(); //will hold the list of the chains for each group
        ...
        //System.out.println("Number of groups : " + numOfGrpups);
        int count = 0;
        ...
        int updatedK=0;
        int g0Size=0;
        if (!flagTop == false) { //Mapping the first node
                ArrayList<Integer> subCands = new ArrayList<Integer>();
                subCands = treemap.get(firstVNF.getNumber());
                //System.out.println("Substrate candidates: " + subCands);
                ...
                head_0 = firstVertex;
                g0Size=groups.get(0).size(); //very first size
                //System.out.println("Size of group 0: " + g0Size);
                groups.get(0).remove(0);
                
                int sizeAfterV0=groups.get(0).size(); //if the size of group zero after maping the first element is zero 
                                                        //we have to start from group one. 
            ...
            for (int k = updatedK; k < groups.size(); k++) { //Going over each group
                   // System.out.print("......................................starting group: " + k
                    ...
                    //System.out.println();
                       ...
                       if (groups.size()>1) //Special case when the size g1 is one
                    {
                     sizeg1 = groups.get(1).size();
                    }

if (k==1 &&(g0Size == 1) && (groups.get(1).size()>0) &&(sizeg1==groups.get(1).size())){
//if (k==0 &&(usedSub.size() == 1)&& (groups.get(0).size()==0) && (groups.get(1).size()>0) &&(sizeg1==groups.get(1).size()))
//if (k==0 &&(usedSub.size() == 1)&& (groups.get(0).size()==0) && (groups.get(1).size()>0) &&(sizeg1==groups.get(1).size())){//newer

    //System.out.println("Catched special case: g0 has one element, both head and tail are the same thing");


    //System.out.println("**********latest used substrate node : " + usedSub);
    // Rest of the elements, starting group1, element 1, Now we just get the distance
    // to the updated tail
    ...
    ArrayList<Vertex> ch1g1 = new ArrayList<Vertex>(); //Create chain 1 for group 1
    ch1g1.add(removeVertexNodeRest);
    mappedChain.add(ch1g1);

    /*if (k == 0) {

        mappedChain.add(chk1);
...
////System.out.println("herrr");
    ...
    if (usedSub.size() == 1) { // Just one VNF is mapped up to now
            ...
            selectedVNF = getNextNodeOtherGroups(SP, treemap, groups.get(0), bw, getNodeVer(usedSub.get(0)),
                            flagTop); // After this function we know which node is mapped next and we have its SP
    ...
    // For each element in group 0 get the distance from head and tail
            ...
            selectedVNF = getNextNodeFirstGroup(SP, treemap, groups.get(0), bw, getNodeVer(usedSub.get(0)),
                            getNodeVer(usedSub.get(usedSub.size() - 1)), flagTop);
    ...
    for (int i = 0; i < groups.get(0).size(); i++) {
            //System.out.println(" Updated groups 0: " + groups.get(0).get(i).getNumber());
    }
...
// we find the SP to the both head and
    //System.out.println("For the very first element of group one:");															// tail of the group 0

    selectedVNF_g1 = getNextNodeFirstGroup(SP, treemap, groups.get(1), bw, getNodeVer(usedSub.get(0)),
...
//                                        //System.out.println("usedSubackup02222222222" + usedSubackup);
    if (temp1g1 == usedSub.get(0) ) { // SP was found to the head of the current usedSub, so the usedSub
                                                                            // should be updated from the start by adding to element 0
            // usedSub.add(0, temp2g2);
            // Direction of chain is decided here

            //System.out.println("usedSubackup before adding used sub" + usedSubackup);
           //System.out.println("usedSub before adding used sub" + usedSub);

           // usedSubackup=usedSub;
            usedSubackup.clear();
            usedSubackup.addAll(usedSub);

            ////System.out.println("usedSubackup after adding used sub" + usedSubackup);
            Collections.reverse(usedSub);
            //System.out.println("usedSubackup after" + usedSubackup);
            usedSub.add(temp2g2);
            //System.out.println("usedSub usedSub" + usedSub);
            usedSubackup.add(temp2g2);
            //System.out.println("usedSubackup usedSub1" + usedSubackup);
   ...
   //System.out.println("tempCostG1" +tempCostG1);
    ...
    //System.out.println("For the rest of the node after mapping the first element of group 1");
    //System.out.println("**********Used substrate nodes till now : " + usedSub);
    ...
    //System.out.println("Current UsedSub for the rest: " + usedSub);
                ...
                if (usedSub.size() == VNFs.size()) {
                    //////System.out.println("finalCostDC: " + Arrays.toString(finalCostDC));
                    ...
                    //System.out.println("Final used substrate nodes: usedSub: " + usedSub);
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        for (int j = 0; j < mappedChain.get(i).size(); j++) {
                            ...
                            //System.out.println("mappedChain: " + i + " Current element: "
                                 //   + mappedChain.get(i).get(j).getNumber());
                        }
                    ...
                    for (int i = 0; i < mappedChainFinal.size(); i++) {
                        for (int j = 0; j < mappedChainFinal.get(i).size(); j++) {
                            ...
                            if (i == 0) {
                                if (!(usedSub.get(0) == mappedChainFinal.get(i).get(0).getNumber())) {

                                    //System.out.println("we have to swap");
                                    ArrayList<Vertex> test = new ArrayList<Vertex>();
                                    int chain0Size = mappedChainFinal.get(i).size();
                                    mappedChainFinal.get(i).clear();
                                    for (int l = 0; l < chain0Size; l++) {
                            ...
                            //System.out.println("mappedChain in swap: " + i + " Current element: "
                    ...
                    ArrayList<Integer> fullPathBeforeTuneUp = findFullPathForDC(mappedChainFinal, Vertices2, Edges2, VNFs, filename,bw,treemapFinal, usedSub, count,this.substrateNet);

                    finalCostDC[0] = (fullPathBeforeTuneUp.size()-1)*bw;

//                    for (graphEdge edge :
//                            Edges) {
//                        //////System.out.println("DC-Edge " + edge.getId() + ":\t BW:" + edge.getBW() + ", Source-Destination: " + edge.getSource() + " - " + edge.getDestination() + ", Start-End Index: " + edge.getStartIndex() + " - " + edge.getEndIndex());
//                    }

                    
                    TuneUpWithConv TTU = new TuneUpWithConv(mappedChainFinal, Vertices2, Edges2, VNFs, filename, bw, treemapFinal,
                            usedSub, count, this.substrateNet);
                    ArrayList<Integer> finalChain = new ArrayList<Integer>();

                   //System.out.println("DC-FinalPath:\t"+Arrays.toString(fullPathBeforeTuneUp.toArray()));
...
//                        for (graphEdge edge :
//                                Edges2) {
//                            //////System.out.println("Tune-Up-Edge " + edge.getId() + ":\t BW:" + edge.getBW() + ", Source-Destination: " + edge.getSource() + " - " + edge.getDestination() + ", Start-End Index: " + edge.getStartIndex() + " - " + edge.getEndIndex());
//                        }

                      //System.out.println("******************************");
                     //System.out.println("Mapped chain before Tuneup process:\t" + Arrays.toString(mappedChainFinal.toArray()));
                     //System.out.println("DC-FinalPath:\t"+Arrays.toString(fullPathBeforeTuneUp.toArray()));
                      //System.out.println("finalCostDC: " + finalCostDC[0] + " for the BW: " + this.bw );
                     //System.out.println("******************************");
                   //     //System.out.println("Final used before TTU: " + usedSubackup);
                    ...
                    if (finalCostDC[1] > finalCostDC[0]) {
                        System.out.println("DC-FinalPath with Conv:\t"+Arrays.toString(fullPathBeforeTuneUp.toArray()));
                        System.out.println("TTU-FinalPath with Conv:\t"+Arrays.toString(finalChainNodes.toArray()));
                        System.out.println("TTU is greater with Conv!!!!" + "Before TTU:" + finalCostDC[0] + " after TTU: " + finalCostDC[1] + " File Name: " + this.filename);
                    }
                    if (finalCostDC[1]==finalCostDC[0]) {
                      //  //System.out.println("DC-FinalPath:\t"+Arrays.toString(fullPathBeforeTuneUp.toArray()));
                       // //System.out.println("TTU-FinalPath:\t"+Arrays.toString(finalChainNodes.toArray()));
                        //System.out.println("TTU is equal!!!!" + "Before TTU:" + finalCostDC[0] + " after TTU: " + finalCostDC[1] + " File Name: " + this.filename);
                    }
                    else {
                        //System.out.println("DC-FinalPath:\t"+Arrays.toString(fullPathBeforeTuneUp.toArray()));
                        //System.out.println("TTU-FinalPath:\t"+Arrays.toString(finalChainNodes.toArray()));
                        //System.out.println("Before TTU:" + finalCostDC[0] + " after TTU: " + finalCostDC[1] + " File Name: " + this.filename);
                    }
                    return finalCostDC;
                }
                //}
    ...
    private ArrayList<Integer> findFullPathForDC(ArrayList<ArrayList<Vertex>> mappedChainFinal, ArrayList<Vertex> vertices2, ArrayList<graphEdge> edges2, ArrayList<functionnode> vnFs, String filename, int bw, TreeMap<Integer, ArrayList<Integer>> treemapFinal, ArrayList<Integer> usedSub, int count, String substrateNet) {
        ...
        //System.out.println("Connected Chain for DC:\t"+Arrays.toString(connectedChain.toArray()));
        ...
        findShortestPathWithConv shortestPath = new findShortestPathWithConv(Vertices2, Edges2, this.VNFs,this.substrateNet);
            ...
            //////System.out.println("DC-Path from " + connectedChain.get(i)+" to "+ connectedChain.get(i+1)+":\t"+Arrays.toString(currentPath.toArray()));

            path.addAll(currentPath);
        }

        // remove duplication
        //System.out.println("DC********: Size of path  w/ duplication:\t" + Arrays.toString(path.toArray()));
        ...
        //System.out.println("DC*********************: Size of path  w/o duplication:\t" + Arrays.toString(path.toArray()));
